...
public class PicAutoPlayer extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		iViewTop.setImageURI(Uri.parse(Constants.files.get(index).getAbsolutePath()));
		
		vSwitcher.addView(iViewTop);
		
		iViewSecond.setImageURI(Uri.parse(Constants.files.get(index).getAbsolutePath()));
				...
				while(true){
					try {
						...
						if(current == 0){
							...
							iViewSecond.setImageURI(Uri.parse(Constants.files.get(getNextIndex()).getAbsolutePath()));
							vSwitcher.removeViewAt(1);
							vSwitcher.addView(iViewSecond);
							vSwitcher.showNext();
						}else{
							iViewTop = (ImageView) vSwitcher.getChildAt(0);
							iViewTop.setImageURI(Uri.parse(Constants.files.get(getNextIndex()).getAbsolutePath()));
